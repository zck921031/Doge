...
public final class FlowdataII
{
	...
	public static double[] turn_rate = new double[]{0.2,0.2,0.6};///{L,R,S}
	public static int[][]  through_rate ;///It's True
	public static int[]    through_level ;//0,1,2
	...
	public void initTrafficLogic()///define array and build logic of roads
	{
		...
		*/
		...
		for(int id=1;id<roadNum;id++)
		{
			...
			if (GotoID[id][0]<=0) {
				trueTurnRate[id][1]  = turn_rate[1]*2;
				trueTurnRate[id][0]  =  0;
			}else if (GotoID[id][1]<=0) {
				trueTurnRate[id][0]  = turn_rate[0]*2;
				trueTurnRate[id][1]  = 0;
			}else if (GotoID[id][2]<=0) {
				trueTurnRate[id][0]  = 0.5;
				trueTurnRate[id][1]  = 0.5;
				trueTurnRate[id][2]  = 0;
			}
		}
		...
		{
			if(hasRoadID[id]==false) continue;
			for(int i=0;i<3;i++) through_rate[id][i] = (int)((double)(20-through_level[id]*5)*trueTurnRate[id][i]);
		}
		//Test:
/*		
		for(int id=1;id<roadNum;id++)
...
*/		
	...
	public int CalcuRoadStay(int roadid,int timID)/// the cars can't leave at timID of roadid
	{
		...
		if (history_trafficlight[roadid][0][timID]==1) {
			leftThrough = through_rate[roadid][0];
		}
		if (history_trafficlight[roadid][1][timID]==1) {
			rightThrough = through_rate[roadid][1];
		}
		if (history_trafficlight[roadid][2][timID]==1) {
			straightThrough = through_rate[roadid][2];
		}
		int tmpFlow = roadFlow[roadid][timID];
		int wantGoLeft  = (int)Math.floor(tmpFlow * trueTurnRate[roadid][0]);
		int wantGoRight = (int)Math.floor(tmpFlow * trueTurnRate[roadid][1]);
		int wantGoStraight = tmpFlow - wantGoLeft - wantGoRight;
		if(GotoID[roadid][2]<=0)
		{
			wantGoRight = tmpFlow-wantGoLeft;
			wantGoStraight = 0;
		}
		//change:
		int leftStay     = Math.max(0, wantGoLeft     - leftThrough     );
		int rightStay    = Math.max(0, wantGoRight    - rightThrough    );
		int straightStay = Math.max(0, wantGoStraight - straightThrough );
		...
		else
		{
			...
			for(int i=0;i<roadNum;i++)
			{
				...
				///change : calculate how many cars left the map from the exit!
				{
					...
					{
						int tmpFlow = roadFlow[leftid][timID-1];
						int wantGoLeft  = (int)Math.floor(tmpFlow * trueTurnRate[leftid][0]);
						leftIn = Math.min(through_rate[leftid][0],wantGoLeft);
					}
					if(rightid>0  && history_trafficlight[rightid][1][lastTimID]==1)
					{
						int tmpFlow = roadFlow[rightid][timID-1];
						int wantGoLeft  = (int)Math.floor(tmpFlow * trueTurnRate[rightid][0]);
						int wantGoRight = (int)Math.floor(tmpFlow * trueTurnRate[rightid][1]);
						if(GotoID[rightid][2]<=0)
						{
							wantGoRight = tmpFlow-wantGoLeft;
						}
						rightIn = Math.min(through_rate[rightid][1],wantGoRight);
					}
					if(straightid>0  && history_trafficlight[straightid][2][lastTimID]==1)
					{
						int tmpFlow = roadFlow[straightid][timID-1];
						int wantGoLeft  = (int)Math.floor(tmpFlow * trueTurnRate[straightid][0]);
						int wantGoRight = (int)Math.floor(tmpFlow * trueTurnRate[straightid][1]);
						int wantGoStraight = tmpFlow - wantGoLeft - wantGoRight;
						if(GotoID[straightid][2]<=0)
						{
							wantGoRight = tmpFlow-wantGoLeft;
							wantGoStraight = 0;
						}
						straightIn = Math.min(through_rate[straightid][2],wantGoStraight);
					}
