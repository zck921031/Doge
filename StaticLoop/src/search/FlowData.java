...
public final class FlowData
{
	...
	public int roadID[][];
	public int roadLinktl[][];///a direct road : from u=roadLinktl[id][0] to v=roadLinktl[id][1] ( iff roadID[u][v] = id )
	public ArrayList<Integer> lightLinkRoad[];/// A Light will connect to at most 4 roads.
	...
	public void initTrafficLogic()///define array and build logic of roads
	{
		...
		lightLinkRoad = new ArrayList[tlNum];
		...
		for (int i=0; i<tlNum; i++){
			lightLinkRoad[i] = new ArrayList<Integer>();
		}
		///Build traffic logic!!
		for(String light:lights)
		{
			...
			lightLinkRoad[atID].add( Integer.valueOf(thisRoad) );
			//System.out.println(""+atID+" : "+RoadString(thisRoad));
//			for(int i=0;i<4;i++)
//			{
//				if(lightLinkRoad[atID][i]==-1)
//				{
...
*/		
	...
	public double CalcRoadPenalty(int id, int TimID)
	{
		if(hasRoadID[id]==false) return 0;
	...
	public double updataPenalty(int TimID)
	{
		...
		double tmpSum = 0;
		for(int id=0;id<roadNum;id++){
			double tFlowStay = CalcRoadPenalty(id, TimID);
